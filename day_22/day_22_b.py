...
# If the current node is flagged, the virus reverses direction, and will go back the way it came.
def reverse_direction(input_direction):
    if input_direction == "S":
        return "N"
    elif input_direction == "N":
        return "S"
...
# iteratively pad an empty border around the central map
k = 0
while True:
...
# In this part of the puzzle, we simply refine our if, elif, else conditions further to accommodate for Weakened and
# Flagged nodes amongst clean and infected ones.  In addition to this, we also add the reverse_direction function.

# Clean nodes become Weakened.
# Weakened nodes become Infected.
# Infected nodes become Flagged.
# Flagged nodes become Clean.
# If the current node is clean, the virus turns left.
# If the current node is Weakened, it does not turn, and will continue moving in the same direction.
# If the current node is infected, it turns right.
# If the current node is flagged, it reverses direction, and will go back the way it came.
...
cause_infection = 0
n_bursts = int(10**7)  # Part of your puzzle input.
for i in range(n_bursts):
    if node_map[y_pos, x_pos] == ".":  # The current node is clean
        direction = turn_anticlockwise(direction)
        node_map[y_pos, x_pos] = "W"  # the current node becomes weakened.
    elif node_map[y_pos, x_pos] == "W":  # The current node is weakened
        node_map[y_pos, x_pos] = "#"  # the current node becomes infected.
        cause_infection += 1
    elif node_map[y_pos, x_pos] == "#":  # The current node is infected.
        direction = turn_clockwise(direction)
        node_map[y_pos, x_pos] = "F"  # the current node becomes flagged.
    else:
        direction = reverse_direction(direction)  # The current node is flagged
