...
def populate_variants(orig):
    ...
    # Before naively adding all the variations to the rule, we test first whether the variations might be already
    # present in the rule-book.  Otherwise our rule-book becomes redundant as it contains duplicated key:value pairs.
...
for line in f.readlines():
    ...
    left = line[0].strip()
    right = line[1].strip()
    left = "".join(list(filter(lambda char: char != "/", left)))
    right = "".join(list(filter(lambda char: char != "/", right)))
    my_dict[left] = right
f.close()
...
n_iterations = 5  # Part of your puzzle input.  In Part 2 of the puzzle, n_iterations will be equal to 18.
...
number_on_pixels = np.count_nonzero(condition)  # count all the instances in which fractal_array == "#".
