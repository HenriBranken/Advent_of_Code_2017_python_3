...
while True:  # trim blank space surrounding the lhs of the map.
    ...
    if all(lhs == " "):
        ...
        break
while True:  # trim the blank space surrounding the rhs of the map.
    rhs = array[:, -1]
    if all(rhs == " "):
        array = array[:, 0:-1:1]
    else:
        break
while True:  # trim he blank space surrounding the top of the map.
    top = array[0, :]
    if all(top == " "):
        array = array[1:, :]
    else:
        break
while True:  # trim the blank space surounding the bottom of the map.
    bottom = array[-1, :]
    if all(bottom == " "):
        array = array[0:-1:1, :]
    else:
...
while True:
    while char != "+":  # propagate in the current given direction until encountering a "+" sign
        ...
        steps += 1  # the crux of Part 2 of the puzzle.  steps is a growing integer that represents the amount of steps
        # taken by the network packet along the map (puzzle input.)
    ...
    # plausible directions, and continue in the correct direction; i.e. the direction that does not start out with a
    # blank character " ".
    ...
    illegal_inds.append((curr_pos[0], curr_pos[1]))
    steps += 1  # as commented above, steps is the crux of Part 2 of the puzzle.  Step is a growing integer that
    # represents the amount of steps take by the network packet along the map (puzzle input.)
    if char in letters:
        pathway.append(char)

print(str(steps) + " are required to be followed by the network packet.")
# Answer is 18702.
